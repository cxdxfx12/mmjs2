...
// ParsePriceTableFromRows 从 Excel 行数据解析六区参考价
// 列顺序: 区域 | 0~0.5kg | 0.5~1kg | 1~2kg | 2~3kg | 3~30kg首重 | 3~30kg续重 | 30kg+首重 | 30kg+续重
func ParsePriceTableFromRows(rows [][]string) (map[string]ZonePriceScheme, error) {
	if len(rows) < 2 {
		return nil, fmt.Errorf("模板至少需要表头+1条数据")
	}
	startRow := 0
	if len(rows[0]) > 0 {
		h := strings.TrimSpace(rows[0][0])
		if strings.Contains(h, "区域") || strings.EqualFold(h, "zone") {
			startRow = 1
		}
	}
	result := make(map[string]ZonePriceScheme)
	for i := startRow; i < len(rows); i++ {
		row := rows[i]
		if len(row) == 0 {
			continue
		}
		zoneName := strings.TrimSpace(row[0])
		if zoneName == "" {
			continue
		}
		result[zoneName] = ZonePriceScheme{
			Price0_05: parsePriceCell(row, 1),
			Price05_1: parsePriceCell(row, 2),
			Price1_2:  parsePriceCell(row, 3),
			Price2_3:  parsePriceCell(row, 4),
			First3_30: parsePriceCell(row, 5),
			Cont3_30:  parsePriceCell(row, 6),
			First30up: parsePriceCell(row, 7),
			Cont30up:  parsePriceCell(row, 8),
		}
	}
	if len(result) == 0 {
		return nil, fmt.Errorf("未解析到有效价格数据，请检查区域列是否填写")
	}
	return result, nil
}

func parsePriceCell(row []string, idx int) float64 {
	if idx >= len(row) {
		return 0
	}
