...
**推荐（2026-07 已验证有效）**：使用 [rsrc](https://github.com/akavel/rsrc)
...
- 自研 `ico_to_syso.py` 生成的 syso 格式不完整，Go 能链接但 Windows 资源管理器不显示图标
- **改用 `rsrc` 工具重新生成 syso 后问题解决**
...
- 输出文件名建议先 `_build_temp.exe` 再重命名为 `喵喵云结算.exe`（避免 PowerShell 中文路径编码问题）
- Windows 会缓存 exe 图标，更新后需关闭旧进程并 F5 刷新文件夹

**历史排查（ico_to_syso.py，已不推荐单独使用）**：

1. 资源目录 ID 类型条目错误设置了 `0x80000000` 高位
2. group icon entry 字节格式与 Windows 标准不一致

完整记录见：[开发记忆.md](开发记忆.md)
