# 喵喵结算 - 快速构建脚本 (PowerShell)

$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $PSCommandPath
$exeName = (-join [char[]]@(0x55B5, 0x55B5, 0x7ED3, 0x7B97)) + ".exe"
...
if ((Test-Path $icoPath) -and (Test-Path $rsrcExe)) {
  ...
  Write-Host "[喵喵结算] 警告: 未找到图标工具，将使用已有 syso 资源" -ForegroundColor Yellow
}

# 编译前端
Write-Host "[喵喵结算] 编译前端..." -ForegroundColor Cyan
Push-Location "$root\frontend"
npm run build
Pop-Location

# Go 编译 (带图标 + 隐藏 CMD 窗口)
Write-Host "[喵喵结算] Go 编译..." -ForegroundColor Cyan
Push-Location $root
...
Write-Host "[喵喵结算] ✓ 编译完成！$exeName ($([math]::Round($size/1MB,2)) MB)" -ForegroundColor Green

# 清理旧产物
Remove-Item -Force (Join-Path $root "yunfei.exe") -ErrorAction SilentlyContinue

# 清理
Write-Host "[喵喵结算] 清理缓存..." -ForegroundColor Cyan
