  ...
  <div class="rules-page">
        ...
        <template v-if="activeCustomer">
          <div class="right-header">
            <div class="rh-left">
              ...
              <div class="rh-avgweight" v-if="avgWeightRule">
                <span class="aw-base">基准 {{ avgWeightRule.base_weight }}kg</span>
                <span class="aw-step-line">
                  步长 <span class="aw-step">{{ avgWeightRule.step_weight }}kg</span><span class="aw-sep">/</span><span class="aw-price">+¥{{ avgWeightRule.step_price }}</span>
                </span>
                <el-button link type="primary" size="small" class="aw-config-btn" @click="openAvgWeightDlg">
              ...
              <el-button v-else type="primary" size="small" plain class="rh-avgweight-btn" @click="openAvgWeightDlg">
          ...
          <!-- 阶段报价视图 -->
          <div v-if="viewMode === 'zone'" class="zone-view-wrap rules-rainbow-scroll" v-loading="loadingRules">
            <template v-if="customerRules.length > 0">
              <div class="zone-table-container">
                <el-table ref="rulesTableRef" :key="`zone-${showActionCols}`" :data="bracketRules" stripe border size="small" :max-height="ruleTableMaxHeight" scrollbar-always-on native-scrollbar class="zone-big-table rules-compact-table rules-table-panel" :row-class-name="({row}) => row.calc_mode === 'simple' ? 'simple-mode-row' : ''">
                  ...
                  <el-table-column v-if="showActionCols" label="启用" width="60" align="center">
                  ...
                  <el-table-column v-if="showActionCols" label="操作" width="110" align="center">
                    <template #default="{row}">
            ...
            <div v-if="customerRules.length > 0" class="table-scroll-hint">↑↓ 彩虹滚动条浏览全部省份 · 右上角「操作列」开关可显示/隐藏启用与操作</div>
          </div>

          <!-- 列表视图 -->
          <div v-else class="rule-table-wrap rules-rainbow-scroll">
            <el-table ref="rulesTableRef" :key="`list-${showActionCols}`" :data="customerRules" stripe border size="small" :max-height="ruleTableMaxHeight" scrollbar-always-on native-scrollbar class="rules-compact-table rules-table-panel" v-loading="loadingRules">
              ...
              <el-table-column v-if="showActionCols" label="启用" width="60"><template #default="{row}"><el-switch :model-value="Number(row.is_enabled)===1" size="small" @change="(val: boolean) => toggleRule(row, val)"/></template></el-table-column>
              <el-table-column v-if="showActionCols" label="操作" width="120">
                <template #default="{row}">
                  <el-button link type="primary" size="small" @click="openRuleDlg(row)">编辑</el-button>
                  <el-popconfirm title="确定删除？" @confirm="handleDeleteRule(row.id)">
                    <template #reference><el-button link type="danger" size="small">删除</el-button></template>
    ...
    <el-dialog v-model="zoneTemplateDlgVisible" title="按区域模板生成规则" width="900px" destroy-on-close class="zone-template-dlg">
      ...
      <div class="zt-toolbar">
        ...
        <div class="zt-right">
          <el-button size="default" @click="store.downloadSamplePriceTemplate()">下载模板</el-button>
          <el-upload :show-file-list="false" :auto-upload="false" accept=".xlsx,.xls" :on-change="handleImportZonePrice" :disabled="importingZonePrice">
            <el-button size="default" :loading="importingZonePrice">
              <el-icon><Upload /></el-icon>从Excel导入参考价
            </el-button>
          </el-upload>
          <el-button size="default" @click="loadSamplePrice">加载内置参考价</el-button>
        </div>
      </div>
...
const ruleTableMaxHeight = 'calc(100vh - 280px)'
const showActionCols = ref(true)
const rulesTableRef = ref<{ doLayout?: () => void } | null>(null)

function layoutRulesTable() {
  nextTick(() => rulesTableRef.value?.doLayout?.())
}

watch(showActionCols, () => layoutRulesTable())
...
const importingZonePrice = ref(false)
...
async function loadSamplePrice() {
  try {
    const sample = await store.fetchSamplePriceTable()
    buildZonePriceList(sample)
    ElMessage.success('已加载内置参考价')
  } catch {
    ElMessage.error('加载失败')
  }
}

async function handleImportZonePrice(uploadFile: any) {
  importingZonePrice.value = true
  try {
    const r = await store.importSamplePriceTable(uploadFile.raw)
    if (r.ok && r.data) {
      buildZonePriceList(r.data)
      ElMessage.success(`已从 Excel 导入 ${r.count || Object.keys(r.data).length} 个区域参考价`)
    } else {
      ElMessage.error(r.error || '导入失败')
    }
  } catch {
    ...
    importingZonePrice.value = false
  }
}
...
<style scoped>
.rules-page {
  display: flex; flex-direction: column; gap: 8px; max-width: 1400px;
...
/* ====== 左右分栏 ====== */
.rules-main { display: flex; gap: 12px; flex: 1; min-height: 0; }
...
.right-header {
  display: flex; align-items: center; justify-content: space-between;
  padding: 6px 12px; border-bottom: 1px solid #ebeef5;
  background: linear-gradient(135deg, #fafbff, #f5f7ff);
  flex-wrap: wrap; gap: 6px;
}
.rh-left { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; min-width: 0; }
.rh-customer { font-size: 15px; font-weight: 700; color: #303133; }
.rh-actions { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }
.rh-action-toggle {
  display: inline-flex; align-items: center; gap: 6px;
  padding: 0 8px; height: 28px; border-radius: 6px;
  background: #f5f7ff; border: 1px solid #dce1f5;
}
.rh-action-label { font-size: 12px; color: #606266; white-space: nowrap; }
.rh-avgweight {
...
.rh-avgweight-btn { height: 24px; padding: 0 8px; }
...
.zt-right { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; justify-content: flex-end; }
...
.zone-view-wrap,
.rule-table-wrap {
  flex: 1; min-height: 0; padding: 4px 8px 6px;
  display: flex; flex-direction: column; overflow: hidden;
}
.zone-table-container { flex: 1; min-height: 0; }

/* 彩虹滚动条 + 操作列隐藏（scoped 部分保留布局） */
.rules-rainbow-scroll {
...
.table-scroll-hint {
  ...
  color: #909399;
  padding: 4px 0 2px;
  background: linear-gradient(90deg, transparent, #f5f7ff, transparent);
}

/* 紧凑表格 */
.rules-compact-table :deep(.el-table__cell) { padding: 1px 4px; font-size: 12px; }
.rules-compact-table :deep(.el-table__header .el-table__cell) { padding: 3px 4px; font-size: 12px; }
.rules-compact-table :deep(.el-table__row) { height: 22px; }
.rules-compact-table :deep(.el-tag) { height: 18px; padding: 0 4px; font-size: 11px; }
.rules-compact-table :deep(.el-tag .el-tag__content) { line-height: 16px; }
.rules-compact-table :deep(.el-switch) { height: 16px; line-height: 16px; }
.rules-compact-table :deep(.el-switch__core) { min-width: 28px; height: 16px; }
.rules-compact-table :deep(.el-button.is-link) { padding: 0 2px; height: auto; font-size: 12px; }
.zone-big-table :deep(.el-table__cell) { padding: 1px 4px; }
...
.avgweight-cell {
  ...
  justify-content: center; gap: 6px; line-height: 1.2; white-space: nowrap;
  font-size: 12px;
}
.aw-base { font-weight: 600; color: #409eff; }
.aw-step-line { color: #909399; }
.aw-step { color: #e6a23c; font-weight: 500; }
.aw-sep { margin: 0 1px; }
.aw-price { color: #67c23a; font-weight: 500; }
.aw-config-btn { padding: 0 2px; height: auto; }
.aw-empty { color: #c0c4cc; }
.aw-scope-tag {
  font-size: 11px; padding: 0 5px; border-radius: 10px;
  background: #ecf5ff; color: #409eff; line-height: 18px;
}
...
<!-- 非 scoped：彩虹滚动条 + 启用/操作列隐藏（Element Plus 表格 DOM 需全局选择器） -->
<style>
.rules-rainbow-scroll .el-scrollbar__bar.is-vertical {
  width: 14px !important;
  right: 0 !important;
  border-radius: 8px;
  background: linear-gradient(180deg, #eef2ff, #fce7f3) !important;
  opacity: 1 !important;
}
.rules-rainbow-scroll .el-scrollbar__bar.is-horizontal {
  height: 14px !important;
  bottom: 0 !important;
  border-radius: 8px;
  background: linear-gradient(90deg, #eef2ff, #fce7f3) !important;
  opacity: 1 !important;
}
.rules-rainbow-scroll .el-scrollbar__bar.is-vertical .el-scrollbar__thumb {
  background: linear-gradient(180deg, #ef4444, #f97316, #eab308, #22c55e, #3b82f6, #8b5cf6, #ec4899) !important;
  opacity: 1 !important;
  border-radius: 8px !important;
  box-shadow: 0 0 8px rgba(139, 92, 246, 0.55) !important;
}
.rules-rainbow-scroll .el-scrollbar__bar.is-horizontal .el-scrollbar__thumb {
  background: linear-gradient(90deg, #ef4444, #f97316, #eab308, #22c55e, #3b82f6, #8b5cf6, #ec4899) !important;
  opacity: 1 !important;
  border-radius: 8px !important;
  box-shadow: 0 0 8px rgba(139, 92, 246, 0.55) !important;
}
.rules-rainbow-scroll .el-scrollbar__wrap::-webkit-scrollbar {
  width: 14px;
  height: 14px;
}
.rules-rainbow-scroll .el-scrollbar__wrap::-webkit-scrollbar-track {
  background: linear-gradient(180deg, #eef2ff, #fce7f3);
  border-radius: 8px;
}
.rules-rainbow-scroll .el-scrollbar__wrap::-webkit-scrollbar-thumb {
  border-radius: 8px;
  border: 2px solid #eef2ff;
  background: linear-gradient(180deg, #ef4444, #f97316, #eab308, #22c55e, #3b82f6, #8b5cf6, #ec4899);
  box-shadow: 0 0 8px rgba(139, 92, 246, 0.55);
}
.rules-rainbow-scroll .el-scrollbar__wrap::-webkit-scrollbar-thumb:hover {
  background: linear-gradient(180deg, #dc2626, #ea580c, #ca8a04, #16a34a, #2563eb, #7c3aed, #db2777);
}
</style>
